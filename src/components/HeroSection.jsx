...
export function HeroSection() {
    ...
    useEffect(() => {
        const fullText = "Emilis Voropajevas"
        /* Terminal animation */
        const interval = setInterval(() => {
            setDisplayText(prev => {
                const next = prev + fullText[prev.length];
    ...
    return (
        ...
        className='h-dvh bg-cover bg-center overflow-hidden rounded-lg flex items-center md:items-start md:pt-80 justify-center relative [mask-image:linear-gradient(to_bottom, _black_60%, _transparent_100%)]'>
            <div className='relative z-10 mx-auto max-w-4xl px-6 md:px-0 mb-20 md:mb-0 text-center'>
                ...
                <motion.div variants={fadeUp} initial="hidden" animate="visible" transition={{duration: 0.3, delay: 0}}
                className='mb-8 inline-flex items-center gap-2 rounded-full px-4 py-1.5 border-border/50 bg-secondary/50 text-sm backdrop-blur-sm'>
...
/* Adjust main text and description*/ 